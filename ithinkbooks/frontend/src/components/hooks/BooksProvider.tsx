...
const BooksProvider: React.FC<{children: ReactNode}> = ({children}) => {
  ...
  const getThemeBooks = useCallback(() => {
    axiosInstance
      .get(`http://158.160.176.183:8000/theme/${keys[keys.length - 1]}`, {
        headers: {
          'X-CSRFToken': token
        }
      })
  ...
  const getBookById = useCallback(() => {
    axiosInstance
      .get(`http://158.160.176.183:8000/products/${keys[keys.length - 1]}`, {
        headers: {
          'X-CSRFToken': token
        }
      })
  ...
  const getBooks = useCallback(() => {
    axiosInstance
      .get('http://158.160.176.183:8000/products', {
        headers: {
          'X-CSRFToken': token
        }
      })
  ...
  const postReview = useCallback(() => {
    axiosInstance
      .post('http://158.160.176.183:8000/review/', {...newReview, user: account.id}, {
        headers: {
          'X-CSRFToken': token
        }
      })
  ...
  const getBooksBySearchWord = useCallback(() => {
    axiosInstance
      .post('http://158.160.176.183:8000/books_search', {
